...
- [ ] CA-1: `src/config/env.js` expone `mongodbUri`, leído de
...
- [ ] CA-2: `src/config/env.js` expone `frontendUrl`, leído de
  ...
  `[frontendUrl]`.
- [ ] CA-3: `src/config/db.conf.js` deja de hardcodear la URI: importa `env`
...
- [ ] CA-4: `server.js` no se modifica: el guard `isMain`
...
- [ ] CA-5: `Base_Datos_StyleB/.env.example` queda completo y sin secretos
  ...
  archivo.
- [ ] CA-6: `scripts/e2e-server.js` sigue funcionando sin cambios funcionales
...
- [ ] CA-7: `docs/environment-variables.md` existe con la sección "Backend
...
- [ ] CA-8: `docs/render-deployment.md` existe con la sección del servicio
...
- [ ] CA-9: `cd Base_Datos_StyleB && npm test` y `npm run test:coverage` en
  verde, sin bajar el trinquete de cobertura existente.
- [ ] CA-10: No se introduce ninguna dependencia npm nueva (solo `process.env`
...
|---|---|---|
| `env.js`: `mongodbUri` + guarda de producción | | |
| `env.js`: `frontendUrl` + default derivado de CORS | | |
| `db.conf.js` lee `env.mongodbUri` (cierra K02) | | |
| `.env.example` backend actualizado | | |
| `docs/environment-variables.md` (sección backend) | | |
| `docs/render-deployment.md` (sección backend) | | |
| Tests backend sin regresión + trinquete | | |
